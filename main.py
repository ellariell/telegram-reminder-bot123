...
async def fallback(message: Message, state: FSMContext):
    if message.text.lower().startswith("напомни мне"):
        ...
        if parsed:
            ...
            await message.answer(f"✅ Запомнил! Я напомню тебе: {parsed['text']} в {parsed['time']}")
        else:
            await message.answer("⚠️ Не понял формат. Пример: напомни мне завтра в 08:00 перевести деньги")
        return
