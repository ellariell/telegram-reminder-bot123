...
async def handle_answer(message: Message, state: FSMContext):
    if message.text == "✅ Да":
        ...
        log_entry("✅ Пользователь подтвердил утренние таблетки")
    else:
...
async def start(message: Message, state: FSMContext):
    await message.answer("Привет! Я бот-напоминалка и диалоговый собеседник 🤖", reply_markup=kb)
...
async def done(message: Message):
    ...
    await message.answer("Записал! Ты молодец 💪")

@dp.message()
async def fallback(message: Message):
    await message.answer("Напиши /start чтобы начать, или жди напоминаний.")

# Задача для автоматических напоминаний и диалога
async def scheduler():
    while True:
        now = datetime.now().time().replace(second=0, microsecond=0)
        for remind_time, text in reminders:
            if now == remind_time:
                try:
                    if remind_time == time(6, 30):
